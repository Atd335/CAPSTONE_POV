...
public class IconButtonFunctions : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0)) { cursorDiff = transform.position - Input.mousePosition; }
        updateSelection();
    }
    ...
    public void a_goToCursor()
    {
        transform.position = Input.mousePosition + cursorDiff;
    }
    ...
    public static void spawnWindowStatic(GameObject windowToSpawn)
    {
        if (windowToSpawn && !GameObject.Find(windowToSpawn.name + "(Clone)"))
        {
            Instantiate(windowToSpawn);
